...
} satisfies Record<IconProps['type'], string>

const ICON_TITLE_MAP: Record<IconProps['type'], string> = {
	info: 'Information',
	success: 'Success',
	warning: 'Warning',
	error: 'Error',
...
const getIcon = ({ color, type, size = 24 }: IconProps) => {
	const colorClass = resolveColorClass(color)
	const inlineColor = resolveInlineColor(color)
	...
	return (
		<svg
			...
			role="img"
		>
			<title>{ICON_TITLE_MAP[type]}</title>
			<path
				strokeLinecap="round"
